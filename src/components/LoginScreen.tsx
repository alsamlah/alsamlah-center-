...
export default function LoginScreen({ pins, roleNames, onLogin, settings }: Props) {
  ...
  const handleNumpad = (n: string) => {
    ...
    if (newPin.length === 4 && selectedRole) {
      if (newPin === pins[selectedRole]) {
        ...
        setTimeout(() => { setError(true); setShake(true); setTimeout(() => { setShake(false); setPin(""); }, 500); }, 200);
      }
    }
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: "var(--bg)" }} dir={isRTL ? "rtl" : "ltr"}>
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold" style={{ color: "var(--accent)" }}>AL<span style={{ color: "var(--text)" }}>SAMLAH</span></h1>
        <p className="text-sm mt-2" style={{ color: "var(--text2)" }}>{t.appSub}</p>
      </div>
      ...
      {!selectedRole ? (
          ...
          <div className="grid gap-3">
            {roles.map((role) => (
              <button key={role} onClick={() => setSelectedRole(role)}
                className="card p-6 text-center transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                style={role === "manager" ? { background: "color-mix(in srgb, var(--accent) 6%, var(--card-bg))", borderColor: "color-mix(in srgb, var(--accent) 15%, transparent)" } : {}}>
                <div className="text-4xl mb-3">{ROLE_ICONS[role]}</div>
                ...
                <div className="text-xs mt-1" style={{ color: "var(--text2)" }}>
                  {isRTL ? ROLE_LABELS[role].ar : ROLE_LABELS[role].en} • {role === "manager" ? t.managerAccess : t.cashierAccess}
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="w-full max-w-xs">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs mt-2" style={{ color: "var(--text2)", opacity: 0.6 }}>{t.enterPin}</p>
          </div>
          ...
          {error && <div className="text-xs text-center mb-4" style={{ color: "var(--red)" }}>{t.wrongPin}</div>}
          <div className="grid grid-cols-3 gap-2.5">
            {["1", "2", "3", "4", "5", "6", "7", "8", "9", "clear", "0", "del"].map((n) => (
              <button key={n} onClick={() => handleNumpad(n)}
                ...
                style={{
                  background: n === "clear" ? "color-mix(in srgb, var(--red) 10%, transparent)" : n === "del" ? "var(--surface2)" : "var(--surface)",
