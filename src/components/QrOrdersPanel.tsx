...
export default function QrOrdersPanel({ tenantId }: Props) {
  const [orders, setOrders] = useState<QrOrder[]>([]);
  const [sound] = useState(() => typeof window !== "undefined" ? new Audio("data:audio/wav;base64,") : null);
  ...
  const fmtTime = (ts: string) => new Date(ts).toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit", hour12: true });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        ...
        {pending.length > 0 && (
          <span className="badge px-3 py-1.5 anim-pulse" style={{ background: "color-mix(in srgb, var(--accent) 12%, transparent)", color: "var(--accent)" }}>
            {pending.length} جديد
          </span>
        )}
      </div>
        ...
        <div className="grid gap-3">
          {orders.map((order) => {
            ...
            <div key={order.id} className="card p-4 transition-all"
              style={{ borderColor: `color-mix(in srgb, ${extend && order.status === "pending" ? "var(--blue)" : statusColor[order.status]} 25%, transparent)`,
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm" style={{ color: extend ? "var(--blue)" : "var(--text)" }}>{order.item_name}</span>
                    {!extend && <span className="badge px-2 py-0.5 text-xs" style={{ background: "var(--input-bg)", color: "var(--text2)" }}>×{order.qty}</span>}
                  </div>
                ...
                <div className="flex flex-col items-end gap-1">
                  {!extend && (
                    <span className="text-sm font-bold flex items-center gap-1" style={{ color: "var(--yellow)" }}>{order.item_price * order.qty} <SarSymbol size={13} /></span>
                  )}
                  <span className="text-[10px] font-semibold" style={{ color: extend && order.status === "pending" ? "var(--blue)" : statusColor[order.status] }}>
